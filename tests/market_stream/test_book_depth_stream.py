...
class TestBookDepthStream(unittest.TestCase):
    ...
    def recv_side_effect(self, *args, **kwargs):
        ...
        if self.sentMessage % 2 == 0:
            ...
            if not self.logOutSent:
                return b"8=FIX.4.4\x019=0000165\x0135=X\x0149=SPOT\x0156=BMDWATCH\x0134=3\x0152=20250301-01:00:00.001000\x01262=DEPTH_STREAM\x01268=1\x01279=1\x01269=0\x01270=638.54000000\x01271=11.76700000\x0155=BNBUSDT\x0125043=7517775\x0125044=7517775\x0110=021\x01"
            else:
    ...
    def test_depth_stream(
        ...
        self.assertEqual(
            b"8=FIX.4.4\x019=129\x0135=V\x0149=BMDWATCH\x0156=SPOT\x0134=2\x0152=20250301-01:00:00.000000\x01262=DEPTH_STREAM\x01263=1\x01264=50\x01266=Y\x01146=1\x0155=BNBUSDT\x01267=2\x01269=0\x01269=1\x0110=021\x01",
            socket_mock.sendall.call_args[0][0],
        )
        ...
        for _ in range(client_md.queue_msg_received.qsize()):
            ...
            if msg.message_type.decode("utf-8") == "X":
                ...
                self.assertEqual("DEPTH_STREAM", subscription_id)
                ...
                for i in range(updates):
                    ...
                    self.assertEqual("0", update_type)
                    self.assertEqual("638.54000000", price)
                    self.assertEqual("11.76700000", qty)
                    self.assertEqual("7517775", last_book_id)
