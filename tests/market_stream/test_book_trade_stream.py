...
class TestBookTradeStream(unittest.TestCase):
    ...
    def recv_side_effect(self, *args, **kwargs):
        ...
        if self.sentMessage % 2 == 0:
            ...
            if not self.logOutSent:
                return b"8=FIX.4.4\x019=0000183\x0135=X\x0149=SPOT\x0156=BMDWATCH\x0134=3\x0152=20250224-11:31:20.047857\x01262=TRADE_STREAM\x01268=1\x01279=0\x01269=2\x01270=640.09000000\x01271=3.13200000\x0155=BNBUSDT\x011003=760268\x0160=20250101-01:00:01.000001\x012446=2\x0110=063\x01"
            else:
    ...
    def test_trade_stream(
        ...
        self.assertEqual(
            b"8=FIX.4.4\x019=122\x0135=V\x0149=BMDWATCH\x0156=SPOT\x0134=2\x0152=20250301-01:00:00.000000\x01262=TRADE_STREAM\x01263=2\x01264=1\x01266=Y\x01146=1\x0155=BNBUSDT\x01267=1\x01269=2\x0110=199\x01",
            socket_mock.sendall.call_args[0][0],
        )
        ...
        for _ in range(client_md.queue_msg_received.qsize()):
            ...
            if msg.message_type.decode("utf-8") == "X":
                ...
                self.assertEqual("TRADE_STREAM", subscription_id)
                ...
                for i in range(updates):
                    ...
                    self.assertEqual("2", update_type)
                    self.assertEqual("640.09000000", price)
                    self.assertEqual("3.13200000", qty)
                    self.assertEqual("760268", trade_id)
                    self.assertEqual("20250101-01:00:01.000001", transact_time)
